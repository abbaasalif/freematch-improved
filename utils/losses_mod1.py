...
class CELoss:
    ...
    def __call__(self, logits, targets, reduction='none'):
        if logits.shape == targets.shape:
            ...
            preds = F.log_softmax(logits, dim=-1)
            # Implement label smoothing within the else branch
            num_classes = logits.size(-1)
            with torch.no_grad():
                # Create smoothed labels when the target is in label form
            ...
            return F.nll_loss(preds, targets.argmax(dim=1), reduction=reduction)
...
class ConsistencyLoss:
    ...
    def __call__(self, logits, targets, mask=None, consistency_weight=0.1, reg_weight=0.01):
        ...
        if mask is not None:
            ...
            weighted_loss = (loss * softmax_weights * mask.float()).mean()
